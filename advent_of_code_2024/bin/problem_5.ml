...
let is_in_right_order ~rules ~ints =
  let int_locs =
    List.mapi ints ~f:(fun i n -> (n, i)) |> Int.Table.of_alist_exn
  in
...
let rec run_dfs ~rules_by_source ~remaining_ints source =
  if not (Hash_set.mem remaining_ints source) then []
  else (
    ...
    Reader.with_file filename ~f:(fun reader ->
        ...
        |> Pipe.iter ~f:(fun s ->
               (if String.contains s '|' then
                  ...
                  if not ints_is_in_right_order then (
                    let ints' =
                      run_topological_sort ~rules:(Queue.to_list rules) ~ints
                    in
