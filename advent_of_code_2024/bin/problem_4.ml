...
let word_with_midpoint_at_position_in_direction ~file_contents ~row ~col ~row' ~col' =
  try
...
let run ~filename =
  let%map file_contents =
    Reader.file_lines filename >>| Array.of_list >>| Array.map ~f:String.to_array
  in
  ...
  Array.iteri file_contents ~f:(fun row line ->
    Array.iteri line ~f:(fun col _char ->
      List.iter increments ~f:(fun (row', col') ->
        word_with_midpoint_at_position_in_direction ~file_contents ~row ~col ~row' ~col'
            ...
            |> List.find ~f:(fun (row'', col'') ->
              Option.exists
                (word_with_midpoint_at_position_in_direction
                   ...
                   ~row':row''
                   ~col':col'')
                ~f:(fun word' -> is_mas word && is_mas word'))
          with
          | Some (row'', col'') ->
            let solution =
              Solution.create
